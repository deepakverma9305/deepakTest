package com.ilabquality.gtaf.guitests.deal.expiry.generic;
...
public class ExpireDealGeneric extends GUITest {

	private GuiTestReportSuite gt;

	public ExpireDealGeneric(GUIController controller) {
		super(controller);
	}
	...
	public void doAction(LogonTests lt, String product,  String prodSubType) {
		...
		try {
			...
			//Expire
			gt.addTest(lt.logonBasic("john","john"));
			gt.addTest(expireDealLocalOutwards(product,"Expiry",prodSubType,aa.getProdRef()));
			gt.addTest(lt.logoutBasic());
			resetEnv(gt);
	...
	public GuiTestReportSingleTest expireDealLocalOutwards(String productType, String step,String productSubType, String prodRef){
		setActiveTest(new GuiTestReportSingleTest());
		getActiveTest().setTestDescription("Expire a deal and Release - Gold Case(" + productType+":" + productSubType +":" + step + " - expireDealLocalOutwards)" );
		System.out.println("Expiry - expireDeal - Gold Case(" + productType+":" + productSubType +":" + step+")");
		getActiveTest().setTestName("Expire " + productType);
		...
		try{
			...
			ts= new TestStep();
			ts.setStepName("Validate the 'Expire' button is enabled.");
			activeStep = ts;
			boolean isEnabled = driver.findElement(caseProductInformation.getPageObjectByID("CaseProductInformation.ExpiryButton").getBy()).isEnabled();
			if  (isEnabled){
				ts.setPassed(true);
				getActiveTest().addTestStep(ts,driver);
				...
				ts.setPassed(false);
				ts.setOptionalFailureMessage("The 'Expiry' button is expected to be enabled, it is disabled");
			...
			activeStep = ts;
			driver.findElement(caseProductInformation.getPageObjectByID("CaseProductInformation.ExpiryButton").getBy()).click();
			ts.setPassed(true);
			getActiveTest().addTestStep(ts,driver);
			...
			activeStep = ts;
			driver.findElement(caseProductInformation.getPageObjectByID("CaseProductInformation.ExpiryQueryOKButton").getBy()).click();
			ts.setPassed(true);
			getActiveTest().addTestStep(ts,driver);
...
// Endd Validation
			...
			sleep(500);
			ts= new TestStep();
			ts.setStepName("Click Proceed on the Application Checks UI");
			activeStep = ts;
			...
			ts= new TestStep();
			ts.setStepName("Click Proceed on the Charges UI");
			activeStep = ts;
			...
			ts= new TestStep();
			ts.setStepName("Click Next on the Review UI");
			activeStep = ts;
			...
			ts= new TestStep();
			ts.setStepName("Validate the value [EXP] exists in the 'Step' field.");
			activeStep = ts;
			...
			if ( val.equalsIgnoreCase("EXP")){
				...
				ts.setPassed(false);
				ts.setOptionalFailureMessage("The value in the 'Step' field["+val+"]+ does not tie up with the expected value[EXP]");
				ts.setTestStepImage(takeScreenshot(driver));
				getActiveTest().addTestStep(ts,driver);
			}
